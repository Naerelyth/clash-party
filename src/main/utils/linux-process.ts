...
export interface LinuxConnectionMetadata {
  network: string
  sourceIP?: string
  sourcePort: string
  processPath?: string
...
async function readSocketOwners(procRoot: string): Promise<Map<string, number[]>> {
  let entries: string[]
  try {
  ...
  const pidEntries = entries.filter((entry) => /^\d+$/.test(entry))
  const owners = new Map<string, number[]>()
  await mapWithConcurrency(pidEntries, 8, async (entry) => {
...
async function buildSocketSnapshot(procRoot: string): Promise<SocketSnapshot> {
  const [endpoints, owners] = await Promise.all([
    readSocketTables(procRoot),
    readSocketOwners(procRoot)
  ])
...
export async function findConnectionPid(
  ...
  for (const inode of inodes) {
    for (const pid of snapshot.owners.get(inode) || []) {
